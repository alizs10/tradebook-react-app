...
const User = ({ plan, iteration, setPlan, handleDelPlan, setDoUserNeedEditPlanWindow, setDoUserNeedDetailsPlanWindow }) => {
    ...
    useEffect(listenForOutsideClicks(listening, setListening, dropdownBtnRef, setShowDropDownBtn, setPlan));
            ...
            <td className="relative" ref={dropdownBtnRef}>
                ...
                {!showDropDownBtn ? null : (
                    <ul
                        ...
                        <li>
                            <button onClick={() => handleDelPlan(plan.id)}
                                className="w-full text-right text-xxxs lg:text-xs py-2 hover:bg-slate-200 dark:hover:bg-slate-700 transition ease-out duration-300">
